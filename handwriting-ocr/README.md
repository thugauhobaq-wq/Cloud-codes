...
на сервер, а тот — в один из двух движков на выбор. Основной — Яндекс: фото
остаётся в России, платится рублями, а для редкого личного пользования
(пара снимков в месяц) цена вопроса — копейки.

| | Yandex Vision OCR (основной) | Claude (Anthropic) |
| --- | --- | --- |
| Качество на сложном почерке | печатный и аккуратный текст читает уверенно, на неразборчивой прописи ошибается чаще | выше: модель опирается на смысл фразы |
| Где обрабатывается фото | дата-центры в России | серверы Anthropic, за рубежом |
| Что нужно завести | аккаунт Yandex Cloud, оплата в рублях | ключ на console.anthropic.com, оплата картой |
| Цена | около 12 копеек за снимок (120 ₽ за 1000 распознаваний) | 1–2 ¢ за фото (`claude-opus-5`) |
| Языки рукописи | русский и английский | любые |
...
```bash
pip install -e .                    # для Claude: pip install -e ".[claude]"
export YC_API_KEY=AQVN... YC_FOLDER_ID=b1g...
...
Нужны две строки в `.env`: `YC_API_KEY` и `YC_FOLDER_ID`. Получаются за
десять минут в консоли [console.yandex.cloud](https://console.yandex.cloud):

1. Войти под Яндекс ID и создать платёжный аккаунт (нужна карта; списания
   идут по факту, за пару снимков в месяц это несколько рублей).
2. В облаке уже есть каталог `default` — его идентификатор вида `b1g…`
   показан на странице каталога. Это `YC_FOLDER_ID`.
3. В каталоге открыть «Сервисные аккаунты» → «Создать», имя любое, роль
   `ai.vision.user` (только распознавание, ничего лишнего).
4. Открыть созданный аккаунт → «Создать новый ключ» → «API-ключ». Показанный
   секрет вида `AQVN…` — это `YC_API_KEY`; консоль показывает его один раз,
   сразу вписать в `.env`.

Проверить ключи, не поднимая сервер:

```bash
YC_API_KEY=AQVN... YC_FOLDER_ID=b1g... python -m hwocr recognize лист.jpg
```

Приложение просит Яндекс не сохранять присланные фото (заголовок
`x-data-logging-enabled: false`). Модель `handwritten` знает русский и
английский; по умолчанию просим только русский, `YC_LANGUAGES=ru,en`
включит оба.
...
| Переменная | Зачем |
| --- | --- |
| `OCR_ENGINE` | `yandex` (по умолчанию) или `claude` |
| `YC_API_KEY`, `YC_FOLDER_ID` | ключ сервисного аккаунта и каталог Yandex Cloud |
| `YC_LANGUAGES` | языки рукописи: `ru` (по умолчанию), `en` или `ru,en` |
...
* **Движки** — за общим интерфейсом `Recognizer.recognize(байты, тип) → текст`
  (`hwocr/recognize/`). Яндекс — один POST на `urllib`, формат сверен с
  исходниками API; Claude — через официальный клиент `anthropic`
  (единственная необязательная зависимость).
...
* 81 тест без сети: оба адаптера на заглушках, очередь, хранилище, HTTP.

## Что не проверено

* **Yandex Vision OCR вживую** — адрес сервиса закрыт прокси среды
  разработки. Формат запроса сверен с исходниками API (`mimeType` — короткое
  имя `JPEG`/`PNG`, модель `handwritten`, языки `ru`/`en`) и примером из
  документации, но ни одного настоящего ответа сервиса адаптер не видел.
  Первая проверка — `python -m hwocr recognize фото.jpg` с вашими ключами;
  если ответ не разберётся, в карточке будет «непонятный ответ Яндекса» и
  подробности в журнале.
* **Claude на настоящем почерке** — ключа в среде разработки не было;
  проверена только цепочка до API с неверным ключом.
...
присланные фото; для продажи услуги этого мало — понадобится договор с
Yandex Cloud как с обработчиком и согласие клиентов.
...
```bash
pip install -e ".[dev]"
pytest -q          # 81 тест, сеть не нужна
ruff check .
```
