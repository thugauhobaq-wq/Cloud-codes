...
#: Кириллица → латиница по ГОСТ-подобной схеме: имя репозитория, домена или
#: файла из русского названия иначе выходит пустым.
_TRANSLIT = {
...
def translit(text: str) -> str:
    ...
    Нужно там, где кириллица не годится: имя репозитория, ветки, каталога.
    Регистр теряется осознанно — все эти имена всё равно пишутся строчными.
    """
    return "".join(_TRANSLIT.get(char, char) for char in (text or "").lower())
