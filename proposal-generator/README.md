...
хочет обсудить. Черновики сохраняются, чтобы следующее КП делать копией
предыдущего.
...
proposals/
  ...
  delivery.py     статусы, журнал и воронка
  mailer.py       письмо с PDF и настройки SMTP
  storage.py      черновики в JSON, поиск по токену ссылки
  server.py       http.server: приватная форма и публичная страница заказчика
  cli.py          команды serve / render / send / status / smtp / demo / drafts / font
...
предложение и что публичная страница не отдаёт ни токен, ни адрес получателя,
ни чужие предложения.
