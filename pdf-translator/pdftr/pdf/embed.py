...
Модуль перенесён из proposal-generator: там он собирает КП, здесь — печатает
перевод поверх исходной страницы. Логика одна и та же, менять её не пришлось.
...
class FontFace:
    ...
    def write(self, pdf: PDFFile, ref: Ref | None = None) -> Ref:
        """Положить шрифт в документ и вернуть ссылку на объект /Font.

        `ref` — заранее зарезервированный номер. Он нужен, когда на шрифт уже
        сослались (страницы пишутся раньше, чем становится известен набор
        использованных глифов).
        """
        ttf = self.ttf
        ...
        return pdf.assign(ref, font) if ref is not None else pdf.add(font)
